...
class ConvertGettersAndSettersToPropertyProcessing : ElementsBasedPostProcessing() {
    ...
    private fun List<KtClassOrObject>.sortedByInheritance(): List<KtClassOrObject> {
        ...
        val state = Array(size) { State.NOT_VISITED }
        val descriptors = map { it.resolveToDescriptorIfAny()!! }
        val descriptorToIndex = descriptors.mapToIndex()
        val outers = descriptors.mapIndexed { i, descriptor ->
            i to descriptor.getAllSuperclassesWithoutAny().mapNotNull { descriptorToIndex[it] }
        }.toMap()

        fun dfs(current: Int) {
            when (state[current]) {
                State.VISITED -> error("Cycle in class hierarchy was found")
                State.NOT_VISITED -> state[current] = State.VISITED
                State.ADDED -> return
            }
            for (outer in outers.getValue(current)) {
                dfs(outer)
            }
            sorted += get(current)
            state[current] = State.ADDED
        }

        while (true) {
            val start = state.indexOfFirst { it == State.NOT_VISITED }
            if (start == -1) break
            dfs(start)
        }
